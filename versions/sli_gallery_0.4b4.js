	 ...
	 *                   SliGallery                   *
	 * ---------------------------------------------- *
	 * File : sli_gallery_0.4b4.js                    *
	 * jQuery plugin creating slider gallery with     *
	 * buttons                                        *
	 * @author Reo  reo.fox@gmail.com                 *
	 * @version 0.4 beta 4 <03-07-2012>               *
	 * @license MIT                                   *
	 * http://opensource.org/licenses/mit-license.php *
	 ** -------------------------------------------- **
	 ** Changelog:                                   **
	 **  - Supports easing animations                **
	 **  - Supports carousel behaviour               **
	 **    (option carousel: true)                   **
	 **  - Improved items loading moment             **
	 **  - Panel added dynamically (usePanel: true)  **
	 **  - Animation at start can be blocked         **
	 **  - When only one element in gallery,         **
	 **    it doesn't start                          **
	 ** -------------------------------------------- **
	*/
	...
	jQuery.fn.extend ({ 
		...
		SliGallery: function (sgConfig)
		{
			...
			var config = {
				...
				animationAtLoad:       true,          // - Play animation at load of slider - 
				
				easing:                "",            // - easing method -
				carousel:              false,
				
				usePanel:              false,         // - use half-opacity text panel -
				panelID:               "promoPanel",  // - panel ID -
			...
			var noGalleryForOne = true; // - When there is only one element in gallery, it doesnt start and pager is hidden -
			...
			function loadItemsArray ()
			{
				...
				// - If using panel, then div is appended -
				if ( config.usePanel )
					jQuery("#" + elementId).append("<div id=\"" + config.panelID + "\"></div>");
				
				// - If only one element in gallery, it doesn't start and pager is hidden -
				
				if ( noGalleryForOne && items.length == 1 ) {
					jQuery("#" + elementId + " " + config.contents + " > " + config.contentsItem).show(0);			// - show this item -
					jQuery("#" + elementId + " " + config.contents).css({ "opacity": "1", "display": "block" });	// - also show contents container -
					jQuery("#" + elementId + " " + config.pager).hide(0);											// - hide pager -
					return;
				}
			...
			function toggleElement (elId, fastToggle)
			{
				...
				jQuery("#" + elementId + " " + config.contents + " > " + config.contentsItem).each( function(index, dom) {
					...
					{
						if ( firstLoad && !config.animationAtLoad ) {
							jQuery(this).hide(0);
						}
				...
				else if ( config.easing != "" )
				{
					jQuery("#" + elementId + " " + config.contents + " > " + config.contentsItem + "#" + item).animate (
						{ left: args.left }, 
						{ duration: ( ( firstLoad && !config.animationAtLoad ) ? 0 : config.showDuration ), easing: config.easing }
					);
				}
				
				else if ( firstLoad && !config.animationAtLoad ) {
					jQuery("#" + elementId + " " + config.contents + " > " + config.contentsItem + "#" + item).show(0);
				}
			...
			function showContents()
			{
				...
				jQuery("#" + elementId + " " + config.contents).animate({ opacity: 1 }, ( ( firstLoad && !config.animationAtLoad ) ? 0 : config.showDuration ) );
				firstLoad = false;
			};
